document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderWeekView() {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            dayHeader.innerHTML = `
                <div>${['T1', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7'][i]}</div>
...
function generateCalendarMulti(containerId) {
  const container = document.getElementById(containerId);
  container.innerHTML = "";

  const daysInMonth = new Date(2024, 5, 0).getDate(); // tháng 5/2024 ví dụ
  const dayNames = ["CN", "T2", "T3", "T4", "T5", "T6", "T7"];
  ...
  for (let i = 1; i <= daysInMonth; i++) {
    ...
    // Bắt sự kiện click để toggle chọn ngày
    dayCell.addEventListener("click", () => {
...
function renderCalendarMulti(containerId, month, year) {
  const container = document.getElementById(containerId);
  container.innerHTML = "";
  ...
  document.getElementById("current-month").innerText = `Tháng ${month + 1}/${year}`;
...
document.getElementById("prev-month").onclick = function() {
  ...
  renderCalendarMulti("calendar-view2", currentMonth, currentYear);
};
...
// Bấm Đặt nhiều ngày thì load lịch
document.getElementById("btn-book-multi").onclick = function() {
  document.getElementById("form-book-time").style.display = "none";
  document.getElementById("form-book-multi").style.display = "block";
  renderCalendarMulti("calendar-view2", currentMonth, currentYear);
};

// Bấm Đặt theo giờ thì ẩn lịch đặt nhiều ngày
document.getElementById("btn-book-time").onclick = function() {
  document.getElementById("form-book-time").style.display = "block";
  document.getElementById("form-book-multi").style.display = "none";
};
